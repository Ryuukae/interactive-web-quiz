...
async function setupResultScreenState(
    ...
    await page.evaluate(
        ({ score, maxScore, isBuilderSource }) => {
            ...
            const returnBuilderBtn =
                document.getElementById("return-builder-btn");
...
async function setupQuizScreenState(
    ...
    await page.evaluate(
        ({
            ...
            const quizActionStack =
                document.getElementById("quiz-action-stack");
            if (quizActionStack) {
                quizActionStack.style.display = isBuilderSource
                    ? "flex"
                    : "none";
            }
        },
...
test.describe("UI Visual Regression Audits", () => {
    ...
    test("Start Screen Baseline", async ({ page }) => {
        ...
        await expect(
            startContainer.locator("#create-quizset-btn")
        ).toBeVisible();
    ...
    test("Creator Screen - Form Layout Integrity", async ({ page }) => {
        ...
        await expect(
            creatorForm.locator("#bulk-import-panel")
    ...
    test("Accordion Component - Format Guide Expanded", async ({ page }) => {
        ...
        await expect(
            page.locator("#bulk-import-panel")
        ...
        await expect(
            formatGuideAccordion.locator(".guide-content")
        ).toBeVisible();
    ...
    test("Quiz Screen - Builder Session Rendering (isBuilderSource)", async ({
        ...
        await expect(
            quizScreen.locator("#quiz-return-builder-btn")
        ).toBeVisible();
    ...
    test("Results Screen - Perfect Score Rendering (Standard Source)", async ({
        ...
        await expect(resultsScreen.locator("#result-message")).toHaveText(
            "100%"
        );

        // Standard flow: Return to start button visible, builder/download hidden
        await expect(
            resultsScreen.locator("#return-start-btn")
        ).toBeVisible();
        await expect(
            resultsScreen.locator("#return-builder-btn")
        ).toBeHidden();
        await expect(
            resultsScreen.locator("#btn-export-results")
        ).toBeHidden();

        await expect(resultsScreen).toHaveScreenshot(
            "result-screen-perfect.png"
        );
    });
