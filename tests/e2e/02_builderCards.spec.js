...
test.describe('E2E Spec 2: Dynamic Question Card Builder Operations', () => {
    ...
    test('should allow adding, populating, and removing dynamic question cards', async ({ page }) => {
        ...
        const correctInput = cards.first().locator('.answer-row.correct-row input[type="text"]');
        await correctInput.fill('Au');

        // Fill distractor answer field
        const distractorInput = cards.first().locator('.answer-row.distractor-row input[type="text"]').first();
