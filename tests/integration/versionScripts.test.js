...
describe("Versioning Scripts Integration Tests", () => {
  ...
  const enforceScriptPath = path.resolve(
    process.cwd(),
    "scripts/enforce-version-bump.js"
  );
  ...
  describe("verify-versions.js", () => {
    it("should exit with 0 when all files match package.json version", () => {
      ...
      try {
        const output = execSync(`node "${verifyScriptPath}"`, {
          cwd: tempDir,
          encoding: "utf8"
        });
        expect(output).toContain(
          "✅ All 1 files successfully matched version 1.5.0"
        );
      } catch (error) {
    ...
    it("should exit with 1 when a file is missing the @version tag", () => {
      ...
      try {
        execSync(`node "${verifyScriptPath}"`, {
          cwd: tempDir,
          encoding: "utf8",
          stdio: "pipe"
        });
  ...
  describe("enforce-version-bump.js", () => {
    it("should update the @version tag in target files to match package.json", () => {
      ...
      try {
        const output = execSync(`node "${enforceScriptPath}" src/update.js`, {
          cwd: tempDir,
          encoding: "utf8"
        });
    ...
    it("should do nothing if the @version tag is already correct", () => {
      ...
      try {
        const output = execSync(`node "${enforceScriptPath}" src/correct.js`, {
          cwd: tempDir,
          encoding: "utf8"
        });
